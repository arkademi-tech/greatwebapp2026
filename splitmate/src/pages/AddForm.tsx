...
export function AddForm({ editTarget, setEditTarget, setPage, showToast }: AddFormProps) {
  const { members, addExpense, updateExpense } = useApp();
  ...
  const submit = async (ev: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const data = { ...form, amount: Number(form.amount.replace(/\D/g, '')) };
      if (isEdit) await updateExpense(editTarget.id, data);
        ...
        <div className="grid grid-cols-2 gap-5">
          ...
          <div className="flex flex-col">
            ...
            <div className="relative">
              ...
              <input value={form.amount} onChange={e => set('amount', formatAmount(e.target.value))}
                placeholder="0"
          ...
          <div className="flex flex-col">
            ...
            <div className="flex gap-3 flex-wrap p-2.5 rounded-lg bg-white"
                ...
                return (
                  <label key={m} className="flex items-center gap-1.5 cursor-pointer">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`text-xs ${on ? 'font-semibold text-blue-600' : 'font-normal text-slate-500'}`}>{m}</span>
                    </div>
                  </label>
                );
              })}
          ...
          <div className="col-span-2 flex gap-2.5 justify-end pt-2 border-t border-slate-200">
            ...
            <Button type="submit" icon={isEdit ? 'check' : 'plus'} disabled={submitting}>
