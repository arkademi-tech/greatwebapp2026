...
import { CATS, CAT_COLORS, MEMBERS } from '../data';
      ...
      {data.map(d => (
        <div key={d.label} className="flex items-center gap-2.5">
          ...
          <div className="flex-1 h-7 bg-slate-50 rounded-md overflow-hidden relative">
            ...
            >
              {(d.val / max) > 0.25 && (
                <span className="text-[11px] font-bold text-white">{fmtK(d.val)}</span>
              )}
            </div>
            {(d.val / max) <= 0.25 && (
              <span
                className="absolute top-1/2 -translate-y-1/2 text-[11px] font-bold text-slate-700"
...
export function Dashboard({ expenses, setPage }: DashboardProps) {
  ...
  return (
    <div className="overflow-auto h-full p-7">
      <PageHeader
        title="Dashboard"
        subtitle="Ringkasan keuangan tim April 2025"
      ...
      <div className="flex gap-4 mb-6 flex-wrap">
        ...
        <StatCard label="Rata-rata/Orang" value={fmt(Math.round(total / MEMBERS.length))} sub="bulan ini" icon="user" color="#D97706" />
      </div>
      ...
      <div className="grid grid-cols-2 gap-5 mb-5">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
          <div className="flex justify-between items-center mb-5">
            ...
            <Badge color="grey">April 2025</Badge>
          </div>
