...
export function ExpenseList({ expenses, setExpenses, setPage, setEditTarget, showToast }: ExpenseListProps) {
  ...
  const reload = () => { setLoading(true); setTimeout(() => setLoading(false), 1200); };
    ...
    <div className="overflow-auto h-full p-7">
      ...
      <div className="flex gap-2.5 mb-4 flex-wrap items-center">
        ...
        {['Semua', ...CATS].map(c => (
          <button
            key={c}
            onClick={() => { setCatFilter(c); setPg(1); }}
        ...
        <div className="flex gap-1.5 ml-auto">
          {['Semua', 'Lunas', 'Belum'].map(s => {
            ...
            return (
              <button
                key={s}
                onClick={() => { setStatusFilter(s); setPg(1); }}
      ...
      {loading ? <SkeletonTable /> : rows.length === 0 ? (
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
          ...
          <div className="grid gap-3 px-4 py-2.5 border-b border-slate-200 bg-slate-50 text-[11px] font-bold text-slate-400 uppercase tracking-wider"
            ...
            {['Tanggal', 'Deskripsi', 'Kategori', 'Nominal', 'Dibayar', 'Status', 'Aksi'].map(h => (
              <div key={h}>{h}</div>
            ))}
          </div>
          ...
          {rows.map(e => (
            ...
            >
              ...
              <div>
                <button onClick={() => togglePaid(e.id)} className="bg-none border-none cursor-pointer p-0">
              ...
              <div className="flex gap-1">
                ...
                {Array.from({ length: pages }, (_, i) => i + 1).map(i => (
                  <button
                    ...
                    style={{
                      border: `1px solid ${i === pg ? '#2563EB' : '#CBD5E1'}`,
                      background: i === pg ? '#2563EB' : 'white',
                      color: i === pg ? '#fff' : '#334155',
                    }}
                  >
                    {i}
                  </button>
