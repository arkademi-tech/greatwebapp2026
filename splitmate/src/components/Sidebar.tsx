...
export function Sidebar({ page, setPage, mobile }: SidebarProps) {
  if (mobile) {
    return (
      <nav className="fixed bottom-0 left-0 right-0 h-15 flex border-t z-40" style={{ background: '#0F172A', borderColor: '#334155' }}>
        {NAV.map(n => {
  ...
  return (
    <aside className="flex flex-col w-56 shrink-0 h-full" style={{ background: '#0F172A' }}>
      ...
      <div className="px-5 py-5 border-b" style={{ borderColor: '#1E293B' }}>
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <div className="text-xs mt-0.5" style={{ color: '#64748B' }}>Kelola bersama</div>
          </div>
        </div>
      </div>
      ...
      <div className="px-5 py-4 border-t" style={{ borderColor: '#1E293B' }}>
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-full bg-blue-500 flex items-center justify-center text-white text-xs font-bold">T</div>
          <div>
            <div className="text-white text-xs font-semibold">Tim Freelance</div>
            <div className="text-[10px]" style={{ color: '#64748B' }}>4 anggota</div>
          </div>
        </div>
      </div>
    </aside>
